...
init(Req, {Operations, LogicHandler, ValidatorState}) ->
    ...
    error_logger:info_msg("Processing operation: ~p", [OperationID]),
...
allowed_methods(Req,State = #state{operation_id = 'DeleteOrder'}) ->
    {[<<"DELETE">>], Req, State};

allowed_methods(Req,State = #state{operation_id = 'GetInventory'}) ->
    {[<<"GET">>], Req, State};

allowed_methods(Req,State = #state{operation_id = 'GetOrderById'}) ->
    {[<<"GET">>], Req, State};

allowed_methods(Req,State = #state{operation_id = 'PlaceOrder'}) ->
    {[<<"POST">>], Req, State};
...
is_authorized(Req0,State = #state{operation_id = 'GetInventory' = OperationID,logic_handler = LogicHandler}) ->
    error_logger:info_msg("is_authorized START"),
    From = header,
    Result = openapi_auth:authorize_api_key(LogicHandler,OperationID,From,"api_key",Req0),
    case Result of
        {true, Context, Req} ->
            error_logger:info_msg("Success: Authorized"),
            {true, Req, State#state{context = Context}};
        {false, AuthHeader, Req} ->
            error_logger:info_msg("Not authorized."),
            {{false, AuthHeader}, Req, State}
    end;

is_authorized(Req, State) ->
    error_logger:info_msg("is_authorized DEFAULT START: It returns true."),
    {true, Req, State}.
...
valid_content_headers(Req0,State = #state{operation_id = 'DeleteOrder'}) ->
    Headers = [],
    {Result, Req} = validate_headers(Headers, Req0),
    {Result, Req, State};

valid_content_headers(Req0,State = #state{operation_id = 'GetInventory'}) ->
    Headers = [],
    {Result, Req} = validate_headers(Headers, Req0),
    {Result, Req, State};

valid_content_headers(Req0,State = #state{operation_id = 'GetOrderById'}) ->
    Headers = [],
    {Result, Req} = validate_headers(Headers, Req0),
    {Result, Req, State};

valid_content_headers(Req0,State = #state{operation_id = 'PlaceOrder'}) ->
    Headers = [],
    {Result, Req} = validate_headers(Headers, Req0),
    {Result, Req, State};
...
handle_request_json(Req0,State = #state{operation_id = OperationID,logic_handler = LogicHandler,validator_state = ValidatorState}) ->
    ...
    error_logger:info_msg("handle_request_json START: LogicHandler => ~p~n",[LogicHandler]),
    case openapi_api:populate_request(OperationID, Req0, ValidatorState) of
        {ok, Populated, Req1} ->
            {Code, Headers, Body} = openapi_logic_handler:handle_request(LogicHandler,OperationID,Req1,maps:merge(State#state.context, Populated)),
            %%_ = openapi_api:validate_response(OperationID,Code,Body,ValidatorState),
