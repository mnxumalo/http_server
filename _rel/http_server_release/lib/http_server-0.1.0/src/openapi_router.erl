...
get_paths(LogicHandler) ->
    ...
    PreparedPaths = maps:fold(
        fun(Path, #{operations := Operations, handler := Handler}, Acc) ->
            error_logger:info_msg("Print all paths: ~p, ~p, ~p~n", [Path, Handler, Operations]),
            [{Path, Handler, Operations} | Acc]
        end,
        [],
        group_paths()
    ),
    [{'_',[{P, H, {O, LogicHandler, ValidatorState}} || {P, H, O} <- PreparedPaths]}].
...
prepare_validator() ->
    R = jsx:decode(element(2, file:read_file(get_api_path()))),
    %%error_logger:info_msg("Jsx decode element(2, file:read_file(openapi.json)) ~p~n", [R]),
