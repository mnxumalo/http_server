...
authorize_api_key(LogicHandler, OperationID, From, KeyParam, Req0) ->
    error_logger:info_msg("authorize_api_key START: LogicHandler => ~p~n",[LogicHandler]),
    ...
    case ApiKey of
        ...
        _ ->
            Result = openapi_logic_handler:authorize_api_key(LogicHandler,OperationID,ApiKey),
            case Result of
                {true, Context}  ->
                    {true, Context, Req};
                false ->
...
get_api_key(header, KeyParam, Req) ->
    error_logger:info_msg("get_api_key START: KeyParam => ~p~n",[KeyParam]),
    Headers = cowboy_req:headers(Req),
    error_logger:info_msg("Headers from the request: ~p~n",[Headers]),
    {maps:get(openapi_utils:to_header(KeyParam),Headers,undefined), Req};
